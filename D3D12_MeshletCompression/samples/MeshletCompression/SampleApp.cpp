...
#include "../../res/shader/Compiled/CompressedMeshletAS.inc"
#include "../../res/shader/Compiled/CompressedMeshletMS.inc"
#include "../../res/shader/Compiled/DebugPS.inc"

static const char* kShadingMode[] = {
    "Shading",
    "Normal",
    "Tangent",
    "Binormal",
...
//-----------------------------------------------------------------------------
SampleApp::SampleApp()
: asdx::Application(L"MeshletCompression", 1920, 1080, nullptr, nullptr, nullptr)
{
...
bool SampleApp::OnInit()
{
    ...
    ResCompressedMeshlets compressedMeshlets;
    {
        {
            ...
            assert(meshlets.Tangents  .empty() == false);
            assert(meshlets.TexCoords .empty() == false);
            assert(meshlets.Primitives.empty() == false);


            if (!CreateCompressedMeshlets(meshlets, compressedMeshlets))
            {
                ELOG("Error : CreateCompressedMeshlets() Failed.");
                return false;
            }

            if (SaveCompressedMeshlets("../../res/model/bunny.cmeshlets", compressedMeshlets))
            {
                ILOGA("Info : Save Compressed Meshlets.");
            }
        #else
            if (!LoadCompressedMeshlets("../../res/model/bunny.cmeshlets", compressedMeshlets))
            {
        ...
        if (!m_PositionBuffer.Init(
            pCmdList,
            compressedMeshlets.Positions.size(),
            sizeof(uint16_t3),
            compressedMeshlets.Positions.data()))
        {
        ...
        if (!m_NormalBuffer.Init(
            pCmdList,
            compressedMeshlets.Normals.size(),
            sizeof(uint16_t2),
            compressedMeshlets.Normals.data()))
        {
        ...
        if (!m_TangentBuffer.Init(
            pCmdList,
            compressedMeshlets.Tangents.size(),
            sizeof(uint16_t),
            compressedMeshlets.Tangents.data()))
        {
            ELOG("Error : TangentBuffer Init Failed.");
            return false;
        }

        if (!m_TexCoordBuffer.Init(
            pCmdList,
            compressedMeshlets.TexCoords.size(),
            sizeof(uint16_t2),
            compressedMeshlets.TexCoords.data()))
        {
        ...
        if (!m_VertexIndexBuffer.Init(
            pCmdList,
            compressedMeshlets.VertexIndices.size(),
            sizeof(uint32_t),
            compressedMeshlets.VertexIndices.data()))
        {
        ...
        if (!m_PrimitiveBuffer.Init(
            pCmdList,
            compressedMeshlets.Primitives.size() * sizeof(uint8_t3),
            compressedMeshlets.Primitives.data()))
        {
...

        {
            ...
            desc.Width              = compressedMeshlets.Positions.size() * sizeof(uint32_t) * 3;
            desc.Height             = 1;
            ...
            if (!m_DebugWriteIndexBuffer.Init(&desc, sizeof(uint32_t) * 3))
            {
...

    {
        ...
        {
            MeshletInfo       Info;
            uint32_t3         OffsetPosition;
            uint32_t2         OffsetNormal;
            uint32_t          OffsetTangent;
            uint32_t2         OffsetTexCoord;
        };

        std::vector<MeshletInfoEx> infos(compressedMeshlets.Meshlets.size());
        for(size_t i=0; i<compressedMeshlets.Meshlets.size(); ++i)
        {
            infos[i].Info           = compressedMeshlets.Meshlets[i];
            infos[i].OffsetPosition = compressedMeshlets.OffsetPosition[i];
            infos[i].OffsetNormal   = compressedMeshlets.OffsetNormal[i];
            infos[i].OffsetTangent  = compressedMeshlets.OffsetTangent[i];
            infos[i].OffsetTexCoord = compressedMeshlets.OffsetTexCoord[i];
        }

        if (!m_MeshletBuffer.Init(
            pCmdList,
            infos.size(),
            sizeof(MeshletInfoEx),
            infos.data()))
        {
            ELOG("Error : MeshletBuffer Init Failed.");
            return false;
        }
        m_MeshletBuffer.GetResource()->SetName(L"CompressedMeshletBuffer");
        ...
        params[0].PositionInfo = compressedMeshlets.PositionInfo;
        params[0].NormalInfo   = compressedMeshlets.NormalInfo;
        params[0].TangentInfo  = compressedMeshlets.TangentInfo;
        params[0].TexCoordInfo = compressedMeshlets.TexCoordInfo;
    ...
    if (!m_PositionVB.Init(meshlets.Positions.size() * sizeof(asdx::Vector3), sizeof(asdx::Vector3)))
    {
        ELOG("Error : PositionVB Init Failed.");
        return false;
    }
    auto positions = m_PositionVB.Map<asdx::Vector3>();
    memcpy(positions, meshlets.Positions.data(), meshlets.Positions.size() * sizeof(asdx::Vector3));
    m_PositionVB.Unmap();

    if (!m_NormalVB.Init(meshlets.Normals.size() * sizeof(asdx::Vector3), sizeof(asdx::Vector3)))
    {
        ELOG("Error : NormalVB Init Failed.");
        return false;
    }
    auto normals = m_NormalVB.Map<asdx::Vector3>();
    memcpy(normals, meshlets.Normals.data(), meshlets.Normals.size() * sizeof(asdx::Vector3));
    m_NormalVB.Unmap();

    if (!m_TexCoordVB.Init(meshlets.TexCoords.size() * sizeof(asdx::Vector2), sizeof(asdx::Vector2)))
    {
        ELOG("Error : TexCoordVB Init Failed.");
        return false;
    }

    auto texcoords = m_TexCoordVB.Map<asdx::Vector2>();
    memcpy(texcoords, meshlets.TexCoords.data(), meshlets.TexCoords.size() * sizeof(asdx::Vector2));
    m_TexCoordVB.Unmap();

    std::vector<uint32_t> srcIndices;
    CreateVertexIndices(meshlets, srcIndices);
    if (!m_IndexIB.Init(meshlets.VertexIndices.size() * sizeof(uint32_t)))
    {
        ELOG("Error : IndexIB Init Failed.");
        return false;
    }
    auto dstIndices = m_IndexIB.Map<uint32_t>();
    memcpy(dstIndices, srcIndices.data(), srcIndices.size() * sizeof(uint32_t));
    m_IndexIB.Unmap();

    m_IndexCount = uint32_t(srcIndices.size());
#endif
    ...
    // メッシュシェーダパイプラインステート生成.
    {
        ...
        desc.AS                             = { CompressedMeshletAS, sizeof(CompressedMeshletAS) };
        desc.MS                             = { CompressedMeshletMS, sizeof(CompressedMeshletMS) };
        ...
        if (!m_WireframeStateMS.Init(pDevice, &desc))
        {
            ELOG("Error : PipelineState::Init() Failed.");
            return false;
        }
    ...
    if (!m_MeshSphereShape.Init(pDevice, compressedMeshlets.BoundingSphere.w, 20))
    {
    ...
    m_MeshSphereShape.SetWorld(
        asdx::Matrix::CreateTranslation(
            compressedMeshlets.BoundingSphere.x,
            compressedMeshlets.BoundingSphere.y,
            compressedMeshlets.BoundingSphere.z));
    m_MeshSphere = compressedMeshlets.BoundingSphere;
    ...
    m_MeshletSpheres.resize(compressedMeshlets.Meshlets.size());
    for(size_t i=0; i<compressedMeshlets.Meshlets.size(); ++i)
    {
        if (!m_MeshletSpheres[i].Init(pDevice, compressedMeshlets.Meshlets[i].BoundingSphere.w, 20))
        {
        ...
        m_MeshletSpheres[i].SetWorld(
            asdx::Matrix::CreateTranslation(
                compressedMeshlets.Meshlets[i].BoundingSphere.x,
                compressedMeshlets.Meshlets[i].BoundingSphere.y,
                compressedMeshlets.Meshlets[i].BoundingSphere.z));
    }
    ...
    const float radius = compressedMeshlets.BoundingSphere.w * 3.0f;
    ...
    m_MeshletInfos = std::move(compressedMeshlets.Meshlets);
...
void SampleApp::OnTerm()
{
    ...
    m_DebugWriteIndexBuffer.Term();

    m_PipelineStateMS.Term();
    m_WireframeStateMS.Term();
    m_RootSigMS.Reset();
...
void SampleApp::OnFrameMove(asdx::FrameEventArgs& args)
{
    ...
    ImGui::Combo("Shading Mode", &m_ShadingMode, kShadingMode, _countof(kShadingMode));
...
)
{
    ...
    pCmd->SetGraphicsRootDescriptorTable(ROOT_T2, m_TangentBuffer        .GetSRV()->GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(ROOT_T3, m_TexCoordBuffer       .GetSRV()->GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(ROOT_T4, m_VertexIndexBuffer    .GetSRV()->GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(ROOT_T5, m_PrimitiveBuffer      .GetSRV()->GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(ROOT_T6, m_MeshletBuffer        .GetSRV()->GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(ROOT_T7, m_MeshInstanceBuffer   .GetSRV()->GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(ROOT_U0, m_DebugWriteIndexBuffer.GetUAV()->GetHandleGPU());
