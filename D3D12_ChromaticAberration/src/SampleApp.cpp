...
// Shaders
//-----------------------------------------------------------------------------
#include "../res/shaders/Compiled/ChromaticAberrationPS.inc"
...
bool SampleApp::OnInit()
{
    ...
    // グラフィックスパイプラインステートの初期化.
    {
        ...
        desc.PS                             = { ChromaticAberrationPS, sizeof(ChromaticAberrationPS) };
...
void SampleApp::OnFrameMove(const asdx::App::FrameEventArgs& args)
{
    #if ASDX_ENABLE_IMGUI
    {
        ...
        ImGui::SetNextWindowSize(ImVec2(210, 100), ImGuiCond_Once);
        if (ImGui::Begin(ASDX_U8("パラメータ制御")))
        {
            ImGui::DragFloat(ASDX_U8("強さ"), &m_Power, 0.001f, 0.001f, 10.0f);
            ImGui::DragFloat(ASDX_U8("スケール"), &m_Scale, 0.01f, 0.0f, 10.0f);
...
void SampleApp::OnFrameRender(const asdx::App::FrameEventArgs& args)
{
    ...
    // 倍率色収差描画.
    {
        float invScale = (float)asdx::Max(m_Width, m_Height);
        invScale = 1.0f / invScale;

        Param param = {};
        param.AspectRatio = float(m_Width) / float(m_Height);
        param.Power       = m_Power;
        param.Scale       = m_Scale * invScale;
        param.Width       = uint16_t(m_Width);
        param.Height      = uint16_t(m_Height);
