...
bool SampleApp::OnInit()
{
    ...
    // ガウスブラー初期化.
    if (!m_GaussianBlur.Init(m_Width, m_Height, m_SwapChainFormat))
    {
...
void SampleApp::OnFrameRender(const asdx::App::FrameEventArgs& args)
{
    ...
    // ガウスブラー適用
    {
        ...
        m_GaussianBlur.Dispatch(pCmd, m_Dispersion, m_TextureBG.GetHandleGPU());
    #else
        m_GaussianBlur.Draw(pCmd, m_Dispersion, m_TextureBG.GetHandleGPU());
    #endif
    }
    ...
    // スプライト描画.
    {
        ...
        m_SpriteRenderer.SetTexture(m_GaussianBlur.GetHandleGPU(), m_LinearClamp.GetHandleGPU());
...
{
    m_GaussianBlur.Resize(args.Width, args.Height);
}
