...
private:
    ...
    ComPtr<ID3D12Resource>                 m_pDepthBuffer;                 // ★追加：深度ステンシルバッファです.
    ...
    ComPtr<ID3D12DescriptorHeap>           m_pHeapDSV;                     // ★追加：ディスクリプタヒープです(深度ステンシルビュー).
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE     m_HandleDSV;                    // ★追加：CPUディスクリプタです(深度ステンシルビュー).
