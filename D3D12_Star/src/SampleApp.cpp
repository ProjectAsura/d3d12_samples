...
: asdx::App(L"Sample", 1920, 1080, nullptr, nullptr, nullptr)
{
    ...
    m_DeviceDesc.MaxCpuResourceCount    = 8192;
...
void SampleApp::OnFrameRender(const asdx::App::FrameEventArgs& args)
{
    ...
    // スターエフェクト適用.
    {
        ...
        auto texW = uint32_t(desc.Width);
        auto texH = uint32_t(desc.Height);
        m_StarEffect.Dispatch(pCmd, texW, texH, m_Texture.GetHandleGPU());
        m_StarEffect.CompositeBloom(pCmd, texW, texH, m_BloomEffect.GetGpuHandleSRV());
    }
    ...
    // スプライト描画.
    {
        ...
        m_SpriteRenderer.SetTexture(m_StarEffect.GetHandleSRV(), m_LinearClamp.GetHandleGPU());
