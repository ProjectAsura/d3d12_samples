...
//      ファイルから読み込みします.
//-----------------------------------------------------------------------------
bool ResHDR::Load( const wchar_t* filename )
{
    if ( filename == nullptr )
    {
    ...
    while( 1 )
    {
        ...
        else if ( buf[0] == 'F' )
        {
            ...
            sscanf_s( buf, "FORMAT=%s", format, (int)_countof(format) );
    ...
    if ( scanlineType == SCANLINE_NY_PX )
    {
        ...
        for( int y = (int32_t)m_Height - 1; y >= 0; y-- )
        {
    ...
    {
        // Direct3Dのテクスチャ座標系に合わせるので，Y方向は逆から読み込み.
        for( int y = 0; y < (int32_t)m_Height; y++ )
        {
            if ( !ReadColor( pFile, &m_pPixels[y * m_Width], m_Width ) )
            {
                fclose( pFile );
...
//      画像の横幅を取得します.
//-----------------------------------------------------------------------------
const uint32_t ResHDR::GetWidth() const
{ return m_Width; }

//-----------------------------------------------------------------------------
//      画像の縦幅を取得します.
//-----------------------------------------------------------------------------
const uint32_t ResHDR::GetHeight() const
{ return m_Height; }

//-----------------------------------------------------------------------------
//      露出値を取得します.
//-----------------------------------------------------------------------------
const float ResHDR::GetExposure() const
{ return m_Exposure; }

//-----------------------------------------------------------------------------
//      RGBE形式のピクセルデータを取得します.
//-----------------------------------------------------------------------------
const uint8_t* ResHDR::GetPixels() const
{ return &m_pPixels[0].r; }

//-----------------------------------------------------------------------------
//      デコードしたピクセルを取得します.
//-----------------------------------------------------------------------------
void ResHDR::GetFloatPixels( float** ppPixels ) const
{
    if ( ppPixels == nullptr )
    {
        ELOG( "Error : Invalid Argument." );
    ...
    // メモリを確保.
    auto pPixels = new (std::nothrow) float [ size ];
    ...
    for( uint32_t i=0; i<m_Width*m_Height; ++i )
    {
        if ( m_pPixels[ i ].e )
        {
            ...
            auto val = ldexpf( 1.0f, m_pPixels[ i ].e - int32_t(128+8) );
