...
// Shaders
//-----------------------------------------------------------------------------
#include "../res/shaders/Compiled/FilmGrainPS.inc"
...
bool SampleApp::OnInit()
{
    ...
    // グラフィックスパイプラインステートの初期化.
    {
        ...
        desc.PS                             = { FilmGrainPS, sizeof(FilmGrainPS) };
...
void SampleApp::OnFrameMove(const asdx::App::FrameEventArgs& args)
{
    #if ASDX_ENABLE_IMGUI
    {
        ...
        {
            ImGui::DragFloat(ASDX_U8("標準偏差"), &m_Sigma, 0.001f, 0.5f, 1.0f);
...
void SampleApp::OnFrameRender(const asdx::App::FrameEventArgs& args)
{
    ...
    {
        Param param = {};
        param.Time  = float(args.AppTimeSec);
        param.Sigma = m_Sigma;
