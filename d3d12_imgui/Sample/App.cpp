...
void DescriptorList::Init(uint32_t count)
{
...
void DescriptorList::Term()
{
    std::lock_guard<std::mutex> locker(m_Mutex);
    m_Free.clear();
}

uint32_t DescriptorList::Alloc()
{
    std::lock_guard<std::mutex> locker(m_Mutex);
    auto result = m_Free.front();
    m_Free.pop_front();
    return result;
}

void DescriptorList::Free(uint32_t& value)
{
    std::lock_guard<std::mutex> locker(m_Mutex);
...
bool App::InitD3D()
{
    ...
    // デバイスを生成.
    hr = D3D12CreateDevice(pAdapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(m_Device.GetAddressOf()));
    if (FAILED(hr))
    {
...
bool App::OnInit()
{
...

    {
        ...
        uint32_t index = m_ListRes.Alloc();
        m_HandleCBV = m_HeapRes->GetCPUDescriptorHandleForHeapStart();
        m_HandleCBV.ptr += SIZE_T(index * m_Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
    ...
    // テクスチャ設定.
    {
        ...
        // シェーダリソースビューを生成.
        auto index = m_ListRes.Alloc();
        D3D12_CPU_DESCRIPTOR_HANDLE handle = m_HeapRes->GetCPUDescriptorHandleForHeapStart();
        handle.ptr += SIZE_T(m_Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) * index);
        m_Device->CreateShaderResourceView(m_Texture.Get(), &viewDesc, handle);

        m_HandleSRV = m_HeapRes->GetGPUDescriptorHandleForHeapStart();
        m_HandleSRV.ptr += SIZE_T(m_Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) * index);
    }
...
void App::OnFrameMove()
{
    ...
    // ImGuiをテストで表示.
    {
        if (ImGui::Begin((char*)u8"ウィンドウのテスト"))
        {
...
//      ディスクリプタを確保します.
//-----------------------------------------------------------------------------
void App::AllocDescriptor(ImGui_ImplDX12_InitInfo* info, D3D12_CPU_DESCRIPTOR_HANDLE* outHandleCPU, D3D12_GPU_DESCRIPTOR_HANDLE* outHandleGPU)
{
    auto app = reinterpret_cast<App*>(info->UserData);
    if (app == nullptr)
        return;

    auto index = app->m_ListRes.Alloc();
    (*outHandleCPU) = app->m_HeapRes->GetCPUDescriptorHandleForHeapStart();
    outHandleCPU->ptr += index * app->m_Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    (*outHandleGPU) = app->m_HeapRes->GetGPUDescriptorHandleForHeapStart();
    outHandleGPU->ptr += index * app->m_Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
}

//-----------------------------------------------------------------------------
//      ディスクリプタを解放します.
//-----------------------------------------------------------------------------
void App::FreeDescriptor(ImGui_ImplDX12_InitInfo* info, D3D12_CPU_DESCRIPTOR_HANDLE handleCPU, D3D12_GPU_DESCRIPTOR_HANDLE handleGPU)
{
    auto app = reinterpret_cast<App*>(info->UserData);
    if (app == nullptr || handleCPU.ptr == 0 || handleGPU.ptr == 0)
        return;

    auto head  = app->m_HeapRes->GetCPUDescriptorHandleForHeapStart();
    auto diff  = handleCPU.ptr - head.ptr;

    auto index = uint32_t(diff / app->m_Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
    app->m_ListRes.Free(index);
}