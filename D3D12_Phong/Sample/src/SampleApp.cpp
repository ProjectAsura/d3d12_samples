...
{
    Vector3    Diffuse;        //!< 拡散反射率です.
    float      Alpha;          //!< 透過度です.
    Vector3    Specular;       //!< 鏡面反射率です.
...
bool SampleApp::OnInit()
{
    ...
    // ライトバッファの設定.
    {
        ...
        ptr->LightPosition  = Vector4(0.0f, 50.0f, 100.0f, 0.0);
    ...
    // パイプラインステートの生成.
    {
        ...
        // 頂点シェーダを検索.
        if (!SearchFilePath(L"PhongVS.cso", vsPath))
        {
        ...
        // ピクセルシェーダを検索.
        if (!SearchFilePath(L"PhongPS.cso", psPath))
        {
