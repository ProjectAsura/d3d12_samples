...
#include "../external/asdx12/res/shaders/Compiled/asdxMoviePS.inc"
...
bool SampleApp::OnInit()
{
    ...
    // スプライトレンダラー初期化.
    if (!m_SpriteRenderer.Init(m_Width, m_Height, 4096, 256, m_SwapChainFormat, m_DepthStencilFormat))
    {
    ...
    {
        asdx::fs::path input = "../res/font/MintMono35-Regular_32.fnb";
        asdx::fs::path path;
        if (!asdx::SearchFilePath(input, path))
        {
            ELOGA("Error : File Not Found.");
        ...
        if (!asdx::LoadA(path.string().c_str(), bin))
        {
            ELOGA("Error : Font Load Failed.");
            return false;
        }
...

    {
        ...
        if (!m_MovieTexture.Init(input.string().c_str()))
        {
            ELOGA("Error : MovieTexture::Init() Failed.");
            return false;
        }

        m_MovieTexture.SetLoop(true);

        D3D12_SHADER_BYTECODE shader = { asdxMoviePS, sizeof(asdxMoviePS) };
        if (!m_SpriteRenderer.CreatePipelineState(pDevice, shader, false, m_MoviePipelineState.GetAddress()))
        {
            ELOGA("Error : CreatePipelineState() Failed.");
            return false;
        }
    }
    ...
    m_PlayerPos.x = float(m_Width - m_Width / 4) - 46.0f;
...
void SampleApp::OnFrameRender(const asdx::App::FrameEventArgs& args)
{
    ...
    // 動画テクスチャを描画.
    {
        m_MovieTexture.Update(args.ElapsedTimeSec);
        auto handleSRV0 = m_MovieTexture.GetHandleGPU(0);
        auto handleSRV1 = m_MovieTexture.GetHandleGPU(1);

        m_SpriteRenderer.ChangeBatch(m_MoviePipelineState.GetPtr(), handleSRV0, m_LinearClamp.GetHandleGPU());
        pCmd->SetGraphicsRootDescriptorTable(asdx::SpriteRenderer::SRV1, handleSRV1);
        m_SpriteRenderer.Add(50, 50, m_Width / 2 - 50, m_Height / 2);
        m_SpriteRenderer.Draw(pCmd);

        m_SpriteRenderer.SetColor(1.0f, 1.0f, 1.0f, 1.0f);
        asdx::FontRenderer::Instance().SetScale(1.5f);
        asdx::FontRenderer::Instance().SetEnableOuter(true);
        asdx::FontRenderer::Instance().SetOuterColor(0.5f, 1.0f, 0.5f, 1.0f);
        asdx::FontRenderer::Instance().SetState(m_SpriteRenderer, m_Font);
        asdx::FontRenderer::Instance().Add(m_SpriteRenderer, m_Font, 50, 50 + m_Height/2, u8"Bボタンで弾を発射します\n十字キーまたは左スティックで移動します");
        m_SpriteRenderer.Draw(pCmd);
    }
